...
| 1.4.0 | 2026-06-16 | **Reporte de errores en tarjetas** (asistencia técnica, RF-54…57, ADR 0008): `ReportControl` (botón + popup) en LessonPlayer/ExamPlayer, `lib/queries/reports.ts`, `script-007.sql` (`certdeck_user_question_reports` + RLS) y Edge Function nueva `certdeck-report-create` (entregadas, no aplicadas). |
| 1.5.0 | 2026-06-16 | **Caché de contenido en cliente** (RNF-17, ADR 0009): `lib/cache/contentCache.ts` + `getCourseContentVersion` + carga en `AppShell` con token; `script-008.sql` (`certdeck_course_catalog_version`) y Edge Function nueva `certdeck-content-version` (entregadas, no aplicadas). |
